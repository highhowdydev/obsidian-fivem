...
export default function HandlingEditor() {
	...
	const handleInputChange = (name: string, value: any, type: string, skipVehicleChange?: boolean) => {
		fetchNui("handling/setField", { name, value, type });
		if (!skipVehicleChange) store.dispatch({ type: "handling/setFieldValid", payload: { name, value, type } });
	};
				...
				<div className='flex flex-col gap-4'>
					<div className='title'>
						...
						<p className='text-sm opacity-70'>
							You are modifying the handling files for the {state.vehicle}
						</p>
					</div>
							...
							case "float":
								return (
									<FloatInput
										...
										value={value.value}
										onChange={(value: any, type: string, skipVehicleChange: boolean) => handleInputChange(name, value, type, skipVehicleChange)}
									/>
								);
							case "vector":
								return (
